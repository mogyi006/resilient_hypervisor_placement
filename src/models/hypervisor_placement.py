...
def greedy_max_cover(network_operator=None,
    ...
    if n_hypervisors is None or n_hypervisors < 2 or n_hypervisors > len(C):
        logging.error(
...
def heuristic_repeated(heuristic_name: str = None,
    ...
    if candidate_selection == 'random':
        ...
        logging.info(f"Selected solution ({candidate_selection}): {selected_solution}")
        ...
        selected_solution = min_solutions[accepted_counts.index(max(accepted_counts))]
        logging.info(f"Selected solution ({candidate_selection}): {selected_solution}")
        return selected_solution
